...
namespace LabratEyeTracking
{
    public static class SRanipalHelper
    {
        ...
        public static void Initialize()
        {
            ...
            if (eyeError.VerifyEyeError()) { LogHelper.Error("Failed to Initialize SRanipal Eye Tracking! EXCEPTION: " + eyeError.ToString()); }
            else { LogHelper.Debug("SRanipal Eye Tracking Initialized!"); EyeTrackingEnabled = true;  }
        }

        public static void Kill()
        {
            if (EyeTrackingEnabled) { SRanipal_API.Release(SRanipal_Eye_v2.ANIPAL_TYPE_EYE_V2); LogHelper.Debug("Killed SRanipal"); }
            else { LogHelper.Warn("Eye Tracking is not enabled, can't kill."); }
        }
